  ...
  return (
    <div className="flex items-center gap-2 w-fit">
      <svg
        ...
        className={theme === 'dark' ? 'fill-100' : 'fill-600'}
      >
      ...
      <button
        className="w-12 h-6 flex items-center rounded-full p-[6px] cursor-pointer bg-grey-dark bg-600"
        onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}
      >
        <div
          className={twMerge(
            'bg-white h-3 w-3 rounded-full transform duration-300 ease-in-out',
            theme === 'light' && toggleClass
          )}
        ></div>
      </button>
      <svg
        ...
        height="18"
        xmlns="http://www.w3.org/2000/svg"
        className={theme === 'light' ? 'fill-100' : 'fill-600'}
      >
